...
//    classifyImage(widget.imagePath, total);
  ...
  Future<void> runTextToSpeech(String outputMoney, int totalMoney) async {
    ...
    if (outputMoney == "200 rupees") {
      String tot = totalMoney.toString();
      print(tot);
      String speakString = "Two Hundred rupees";
      await flutterTts.setSpeechRate(0.8);
      await flutterTts.awaitSpeakCompletion(true);
      await flutterTts.speak(speakString);
    }
    ...
    if (outputMoney == "No note found") {
      String tot = totalMoney.toString();
      print(tot);
      String speakString = "Please capture valid Image";
      await flutterTts.setSpeechRate(0.8);
      await flutterTts.awaitSpeakCompletion(true);
      await flutterTts.speak(speakString);
    }
  ...
  classifyImage(String image) async {
    ...
    if (op != null) {
      if (op[0]["label"] == "1 10" || op[0]["label"] == "10 10_1") {
        total += 10;
        runTextToSpeech("10 rupees", total);
      }
      if (op[0]["label"] == "2 20" || op[0]["label"] == "9 20_1") {
        total += 20;
      ...
      if (op[0]["label"] == "4 100"  || op[0]["label"] == "7 1001") {
        total += 100;
